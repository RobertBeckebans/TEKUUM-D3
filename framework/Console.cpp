...
#if 0
	const int gameThreadTotalTime = com_frameMsec; // commonLocal.GetGameThreadTotalTime();
#else
	const int gameThreadTotalTime = time_gameFrame + time_gameDraw;
#endif
	const int gameThreadGameTime = time_gameFrame; //commonLocal.GetGameThreadGameTime();
	const int gameThreadRenderTime = time_gameDraw; //commonLocal.GetGameThreadRenderTime();
	const int rendererBackEndTime = time_backend; //commonLocal.GetRendererBackEndMicroseconds();
	const int rendererShadowsTime = time_shadows; //commonLocal.GetRendererShadowsMicroseconds();
	const int rendererGPUIdleTime = 0; //TODO commonLocal.GetRendererIdleMicroseconds();
...
float idConsoleLocal::DrawAsyncStats( float y )
{
	...
	if( idAsyncNetwork::server.IsActive() )
	{
		...
		DrawTextRightAlign( LOCALSAFE_RIGHT, y, msg.c_str() );
...
float idConsoleLocal::DrawSoundDecoders( float y )
{
	...
	while( ( index = soundSystem->GetSoundDecoderInfo( index, decoderInfo ) ) != -1 )
	{
		...
		DrawTextLeftAlign( LOCALSAFE_LEFT, y, "%3d: %3d%% (%1.2f) %s: %s (%dkB)", numActiveDecoders, percent, decoderInfo.lastVolume, decoderInfo.format.c_str(), decoderInfo.name.c_str(), decoderInfo.numBytes >> 10 );
		numActiveDecoders++;
	}
	return y;
}
