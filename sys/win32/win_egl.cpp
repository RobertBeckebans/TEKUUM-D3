...
void GLW_CheckWGLExtensions( HDC hDC )
{
	...
	glConfig.swapControlTearAvailable = false; //WGLEW_EXT_swap_control_tear != 0;
...
/*
========================
CreateEGLContextOnDC
========================
*/
static bool CreateEGLContextOnDC( const HDC hdc, const bool debugContext )
{
	...
	win32.eglDisplay = eglGetDisplay( hdc );
	if( win32.eglDisplay == EGL_NO_DISPLAY )
	{
	...
	if( !eglInitialize( win32.eglDisplay, &majorVersion, &minorVersion ) )
	{
	...
	EGLint numConfigs;
	if( !eglGetConfigs( win32.eglDisplay, NULL, 0, &numConfigs ) )
	{
	...
	EGLConfig config;
	if( !eglChooseConfig( win32.eglDisplay, configAttribList, &config, 1, &numConfigs ) )
	{
	...
	eglGetConfigAttrib( win32.eglDisplay, config, EGL_RED_SIZE, &redSize );
	eglGetConfigAttrib( win32.eglDisplay, config, EGL_GREEN_SIZE, &greenSize );
	eglGetConfigAttrib( win32.eglDisplay, config, EGL_BLUE_SIZE, &blueSize );
	eglGetConfigAttrib( win32.eglDisplay, config, EGL_ALPHA_SIZE, &alphaSize );
	eglGetConfigAttrib( win32.eglDisplay, config, EGL_DEPTH_SIZE, &depthSize );
	eglGetConfigAttrib( win32.eglDisplay, config, EGL_STENCIL_SIZE, &stencilSize );

	glConfig.colorBits = redSize + greenSize + blueSize + alphaSize;
	glConfig.depthBits = depthSize;
	glConfig.stencilBits = stencilSize;

	win32.eglSurface = eglCreateWindowSurface( win32.eglDisplay, config, ( EGLNativeWindowType ) win32.hWnd, surfaceAttribList );
	if( win32.eglSurface == EGL_NO_SURFACE )
	{
	...
	win32.eglContext = eglCreateContext( win32.eglDisplay, config, EGL_NO_CONTEXT, contextAttribs );
	if( win32.eglContext == EGL_NO_CONTEXT )
	{
...
static bool GLW_InitDriver( glimpParms_t parms )
{
	...
	//glConfig.colorBits = 8; //win32.pfd.cColorBits;
	//glConfig.depthBits = 8; //win32.pfd.cDepthBits;
	...
	//
	common->Printf( "...creating EGL context: " );
	if( !CreateEGLContextOnDC( win32.hDC, r_debugContext.GetBool() ) )
	{
		common->Printf( "^3failed^0\n" );
		return false;
	}
	...
	if( !eglMakeCurrent( win32.eglDisplay, win32.eglSurface, win32.eglSurface, win32.eglContext ) )
	{
...
void GLimp_Shutdown()
{
	...
	if( win32.eglContext )
	{
		...
		retVal = eglDestroyContext( win32.eglDisplay, win32.eglContext ) == EGL_TRUE;
		common->Printf( "...deleting EGL context: %s\n", success[retVal] );
		win32.eglContext = EGL_NO_CONTEXT;
	}

	if( win32.eglSurface )
	{
		retVal = eglDestroySurface( win32.eglDisplay, win32.eglSurface ) == EGL_TRUE;
		common->Printf( "...deleting EGL surface: %s\n", success[retVal] );
		win32.eglSurface = EGL_NO_SURFACE;
	}
...
void GLimp_SwapBuffers()
{
	...
	*/
	...
	eglSwapBuffers( win32.eglDisplay, win32.eglSurface );
}
// RB end
